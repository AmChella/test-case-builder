    ...
    list.forEach((d, di) => {
      ...
      d.testSteps.forEach((s, i) => {
        ...
        (s.validations || []).forEach((v, vi) => {
          ...
          if (!v.selector) errs.push(`Case ${di + 1} - Step ${i + 1} - Validation ${vi + 1}: selector is required.`);
            ...
            <div>
              ...
              {d.testSteps.map((s, si) => (
                <div key={si} className="p-2 border rounded my-2 grid grid-cols-1 md:grid-cols-5 gap-2 bg-blue-50/50">
                  <input className="w-full px-2 py-1 border rounded md:col-span-2" placeholder="stepName" value={s.stepName} onChange={e => changeStep(di, si, { stepName: e.target.value })} />
                  ...
                  <input className="w-full px-2 py-1 border rounded" placeholder="selector or path" value={s.selector || s.path} onChange={e => changeStep(di, si, { selector: e.target.value, path: e.target.value })} />
                  ...
                  <input className="w-full px-2 py-1 border rounded" placeholder="step data (optional)" value={s.data || ''} onChange={e => changeStep(di, si, { data: e.target.value })} />
                  {s.action === 'waitForTimeout' ? (
                    <input className="w-full px-2 py-1 border rounded" type="number" placeholder="waitTime (ms)" value={s.waitTime || 0} onChange={e => changeStep(di, si, { waitTime: Number(e.target.value || 0) })} />
                  ) : (
                  ...
                  {s.action === 'custom' && (
                    <input className="w-full px-2 py-1 border rounded" placeholder="custom logic key" value={s.customName || ''} onChange={e => changeStep(di, si, { customName: e.target.value })} />
                  )}
